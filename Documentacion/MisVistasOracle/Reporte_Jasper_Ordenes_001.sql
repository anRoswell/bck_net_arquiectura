...
AS 
select 
      nvl(NVL(a.id_orden,0),-1) as id_orden
     ,UPPER(NVL(m.nombre,' ')) as localidad
...
--      ,(SELECT ruta_web FROM aire.gnl_rutas_archivo_servidor where id_ruta_archivo_servidor = 77) as Logo
     ...
     , 'v_1.0.14 -- 10/07/2024' as version
...
select 
     ...
     ,c.nic
     , case
            when a.origen = 'OSF' then upper(gtc.descripcion ||' - '|| ts.descripcion)
            else upper(gdvq.descripcion || ' - ' || c.tarifa)
      end as tipo_cliente
...
left join aire.ord_ordenes_dato_suministro      ods on ods.id_orden = a.id_orden
left join aire.gnl_tarifas_subcategoria         ts on ts.codigo_osf = ods.codigo_tarifa
left join aire.gnl_tarifas_categoria            gtc on gtc.codigo_tarifa_categoria = ts.codigo_tarifa_categoria;
...
left join aire_mtr.mtr_articulos art on art.id_articulo = op.id_articulo
...
with t as (
      ...
      left join (
            select
                  og.ID_ORDEN
                  ,LISTAGG(ac.CODIGO || ': ' || nvl(oga.OBSERVACION,''), '; ') || '; Accion RCS: ' || nvl(og.observacion,'') || '; SubAnomalia: ' || nvl(sa.descripcion,'') as ListagFaltante
            ...
            left join aire.ord_subanomalias sa on sa.id_anomalia = og.id_anomalia and sa.id_subanomalia = og.id_subanomalia 
            inner join aire.scr_acciones ac on ac.ID_ACCION = oga.ID_ACCION
            where 
                --og.ID_ORDEN = $P{Ds05IdOrden} and 
                ac.IND_ACTIVO = 'S'
            group by og.ID_ORDEN,og.observacion,sa.descripcion
...
BEGIN
    ...
    (
            ...
            (
                  ...
                  -- select 'ACCION_RCS' as Accion, 'SOAN' as Codigo, 'Imagen' Clasificacion from dual 	        union all
                  ...
                  select 'ANOMALIA' as Accion, 'SOAN' as Codigo, 'Imagen' Clasificacion from dual 
            )
            ,t as 
            (
                  select 
...
BEGIN
    ...
    (
      ...
      (
            select 
                  ...
                  ,case 
                        ...
                        -- when n.codigo = 'SFU' then TO_CHAR(n.identificacion_persona_atiende)
                        when n.codigo = 'SFU' then TO_CHAR(n.identificacion_persona_atiende)--TO_CHAR(n.nic)
                        when n.codigo = 'SFP' then ' '
                  else '0' end as Identificacion
                  ,case 
                        when n.codigo = 'SFT' then n.nombre_contratista_persona 
                        when n.codigo = 'SFTG' then n.nombres_testigo
                        when n.codigo = 'SFU' then n.nombres_persona_atiende
                        -- when n.codigo = 'SFU' then n.nombre_cliente
                        when n.codigo = 'SFP' then case when nvl(n.ind_solicita_asesoria_persona_atiende,'N') = 'S' then 'Se Requirio' else ' ' end --n.nombres_persona_atiende
                  else '' end as Nombre   
                  ,case 
            ...
            (
                  select
                        ...
                        ,cl.nic
                        ,cl.nombre_cliente
                        ,q.ind_solicita_asesoria_persona_atiende
                  from aire.gnl_cuestionarios_instancia_soporte a
                  left join aire.gnl_cuestionarios_instancia gci on gci.id_cuestionario_instancia = a.id_cuestionario_instancia
                  left join aire.gnl_soportes b on a.id_soporte = b.id_soporte
                  left join aire.gnl_tipos_soporte c on b.id_tipo_soporte = c.id_tipo_soporte
                  left join aire.ord_ordenes o on o.id_orden = gci.id_orden                  
                  left join aire.gnl_clientes cl on cl.id_cliente = o.id_cliente
      ...
      select
            ...
            ,t.Codigo
            ,t.Clasificacion
            ,NVL(UPPER(t.Identificacion),' ') AS Identificacion
            ,NVL(UPPER(t.Nombre),' ') AS Nombre
