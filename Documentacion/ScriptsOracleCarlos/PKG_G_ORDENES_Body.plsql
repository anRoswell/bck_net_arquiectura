CREATE OR REPLACE package body "AIRE".pkg_g_ordenes as
        ...
        select json_arrayagg(json_object(*) returning clob)
          ...
          from (
                    select c.descripcion                                            as "tipo"
                         ...
                         , c.id_tipo_orden                                          as "id_tipo_orden"                      
...
procedure prc_registrar_orden_gestion (
        ...
        v_georreferencia        	json_object_t;
	...
	begin
        ...
        begin
            ...
            v_orden_gestion.observacion                           := json_value(e_json_orden, '$.anomalias.observaciones');
            ...
            -- validamos la georrefetencia
            begin
                v_georreferencia := v_json_orden.get_object('georreferencia');
                if not v_georreferencia.has('latitud') or not v_georreferencia.has('longitud') then
                    v_id_log := aire.pkg_p_generales.fnc_registrar_log (v_nombre_up, aire.pkg_g_seguridad.v_tipo_mensaje_error, 'Error al registrar gestion: Objeto de georreferencia invalido');
                    sb_escribir_respuesta(1, 'Error N# '|| v_id_log ||' georreferencia invalida.');
                    return;   
                end if;
                v_orden_gestion.georreferencia := v_georreferencia.to_string;
            exception 
                when others then 
                    v_id_log := aire.pkg_p_generales.fnc_registrar_log (v_nombre_up, aire.pkg_g_seguridad.v_tipo_mensaje_error, 'Error al extraer la georreferencia: '|| e_json_orden);
                    sb_escribir_respuesta(1, 'Error #'||v_id_log||' al extraer la georreferencia.');
                    return;
            end;
        exception 
            when others then 
        ...
        if v_orden_gestion.ind_anomalia = 'N' then
            for c_acciones in (select
                ...
                if c_acciones.id_accion is not null then
...
*/       
    ...
    procedure prc_consultar_cliente_por_nic_nis (		
        ...
        v_id_log		aire.gnl_logs.id_log%type;
        v_respuesta		aire.tip_respuesta :=   aire.tip_respuesta( mensaje     => 'Consulta realizada con exito.', nombre_up   => 'pkg_g_ordenes.prc_consultar_cliente_por_nic_nis');
    ...
    procedure prc_gestionar_orden_asignar_contratista (		
        ...
        v_json_objeto           json_object_t; 
        v_id_log		        aire.gnl_logs.id_log%type;
        v_respuesta		        aire.tip_respuesta :=   aire.tip_respuesta( mensaje     => 'Consulta realizada con exito.', nombre_up   => 'pkg_g_ordenes.prc_gestionar_orden_asignar_contratista');        
    ...
    procedure prc_gestionar_orden_des_asignar_contratista (		
        ...
        v_json_objeto           json_object_t; 
        v_id_log		        aire.gnl_logs.id_log%type;
        v_respuesta		        aire.tip_respuesta :=   aire.tip_respuesta( mensaje     => 'Consulta realizada con exito.', nombre_up   => 'pkg_g_ordenes.prc_gestionar_orden_des_asignar_contratista');        
    ...
    procedure prc_consultar_orden_por_id (		
        ...
        v_json_objeto       json_object_t; 
        v_id_log		    aire.gnl_logs.id_log%type;
        v_respuesta		    aire.tip_respuesta :=   aire.tip_respuesta( mensaje     => 'Consulta realizada con exito.', nombre_up   => 'pkg_g_ordenes.prc_consultar_orden_por_id');
        v_objeto      	    aire.ord_ordenes%rowtype;
        v_id_orden          NUMBER;        
    BEGIN
                ...
                for c_datos in (
                    with tx as (
                        select a.id_orden
                             ...
                             , to_number(json_value('{"longitud":0,"latitud":0}', '$.longitud'), '9999.99999999') longitud_ejecucion
                             , to_number(json_value('{"longitud":0,"latitud":0}', '$.latitud'), '9999.99999999') latitud_ejecucion
    ...
    procedure prc_consultar_contratista_por_identificacion (		
        ...
        v_json_objeto       json_object_t; 
        v_id_log		    aire.gnl_logs.id_log%type;
        v_respuesta		    aire.tip_respuesta :=   aire.tip_respuesta( mensaje     => 'Consulta realizada con exito.', nombre_up   => 'pkg_g_ordenes.prc_consultar_contratista_por_identificacion');
        v_objeto      	    aire.ord_ordenes%rowtype;
    ...
    procedure prc_resumen_global_ordenes (		
        ...
        v_json_objeto       json_object_t; 
        v_id_log		    aire.gnl_logs.id_log%type;
        v_respuesta		    aire.tip_respuesta :=   aire.tip_respuesta( mensaje     => 'Consulta realizada con exito.', nombre_up   => 'pkg_g_ordenes.prc_resumen_global_ordenes');
    ...
    procedure prc_obtener_datos_dummi (		
        ...
        v_json_objeto   json_object_t; 
        v_id_log		aire.gnl_logs.id_log%type;
        v_respuesta		aire.tip_respuesta :=   aire.tip_respuesta( mensaje     => 'Consulta realizada con exito.', nombre_up   => 'pkg_g_ordenes.prc_obtener_datos_dummi');
        v_orden      	aire.ord_ordenes%rowtype;
        --v_a number;
        --v_b number;
        --v_r number;
    ...
    procedure prc_registro_ordenes_masivo_temporal (
        ...
        v_json_orden                json_object_t;  
        v_id_log		            aire.gnl_logs.id_log%type;
        v_respuesta                 aire.tip_respuesta := aire.tip_respuesta(mensaje => 'Registro exitoso.', nombre_up => 'pkg_g_ordenes.prc_registro_ordenes_masivo_temporal');
    ...
    begin
        ...
        begin
            ...
            SET Numero_Orden = to_char(systimestamp, 'DDMMYYYYHH24MISS') || Orden
    ...
    procedure prc_filtros_parametros_Iniciales (
        ...
        v_id_log		            aire.gnl_logs.id_log%type;
        v_respuesta                 aire.tip_respuesta := aire.tip_respuesta(mensaje => 'Registro exitoso.', nombre_up => 'pkg_g_ordenes.prc_filtros_parametros_Iniciales');
        v_json          clob;  
    ...
    procedure prc_rechazar_orden_scr(
        e_json  in  clob, 
        s_json  out clob
    ) is
        v_id_estado_aisgnada_contratista    aire.ord_estados_orden.id_estado_orden%type;
        v_id_estado_aisgnada_tecnico        aire.ord_estados_orden.id_estado_orden%type;
        v_orden                             aire.ord_ordenes%rowtype;
        v_respuesta		                    aire.tip_respuesta := aire.tip_respuesta( mensaje => 'Orden rechazada con exito.', nombre_up => 'pkg_g_ordenes.prc_rechazar_orden');
        v_id_log	                        aire.gnl_logs.id_log%type;
		procedure sb_escribir_respuesta(codigo in number, mensaje in varchar2)
		is begin
			apex_json.free_output; 
    ...
    begin
        v_orden.id_orden        := json_value(e_json, '$.id_orden');

        -- consultamos el estado asignada tecnico
        begin
            select id_estado_orden
              into v_id_estado_aisgnada_tecnico
              from aire.ord_estados_orden
             where codigo_estado = 'SEAS';
        exception
            when others then
                v_id_log := aire.pkg_p_generales.fnc_registrar_log (v_respuesta.nombre_up, aire.pkg_g_seguridad.v_tipo_mensaje_error, 'Error al consultar el estado asignada tecnico: '||sqlerrm);
                sb_escribir_respuesta(1, 'Error N '||v_id_log||' al rechazar orden: no se encontro la parametrizacion del estado de la orden');
                return;
        end;

        -- validamos si la orden existe y esta asignada a tecnico
        begin
            select id_estado_orden
              into v_orden.id_estado_orden
              from aire.ord_ordenes
             where id_orden = v_orden.id_orden;

             -- validamos que para rechazar la orden debe estar asignada a tecnico
             if v_orden.id_estado_orden <> v_id_estado_aisgnada_tecnico then
                v_id_log := aire.pkg_p_generales.fnc_registrar_log (v_respuesta.nombre_up, aire.pkg_g_seguridad.v_tipo_mensaje_error, 'Error al cerrar la orden: la orden no tiene un estado valido');
                sb_escribir_respuesta(1, 'Error N '||v_id_log||' al rechazar orden: la orden no tiene un estado valido');
                return;
             end if;
        exception
            when others then
                v_id_log := aire.pkg_p_generales.fnc_registrar_log (v_respuesta.nombre_up, aire.pkg_g_seguridad.v_tipo_mensaje_error, 'Error al consultar la orden: '||sqlerrm);
                sb_escribir_respuesta(1, 'Error N '||v_id_log||' al rechazar orden: no se encontro la orden');
                return;
        end;        

        -- consultamos el estado asignada contratista
        begin
            select id_estado_orden
              into v_id_estado_aisgnada_contratista
              from aire.ord_estados_orden
             where codigo_estado = 'SASI';
        exception
            when others then
                v_id_log := aire.pkg_p_generales.fnc_registrar_log (v_respuesta.nombre_up, aire.pkg_g_seguridad.v_tipo_mensaje_error, 'Error al consultar el estado rechazada: '||sqlerrm);
                sb_escribir_respuesta(1, 'Error N '||v_id_log||' al rechazar orden: no se encontro la parametrizacion del estado rechazada');
                return;
        end;


        v_orden.id_estado_orden         := v_id_estado_aisgnada_contratista;
        v_orden.id_contratista_persona  := null;
        v_orden.observacion_rechazo     := json_value(e_json, '$.observacion_rechazo');
        v_orden.fecha_rechazo           := to_date(json_value(e_json, '$.fecha_rechazo'), 'DD/MM/YYYY HH24:MI:SS');

        -- actualizamos la orden a estado rechazada
        update aire.ord_ordenes 
           set id_estado_orden          = v_orden.id_estado_orden
             , id_contratista_persona   = v_orden.id_contratista_persona
             , observacion_rechazo      = v_orden.observacion_rechazo 
             , fecha_rechazo            = v_orden.fecha_rechazo
           where id_orden = v_orden.id_orden;
    ...
    exception
        when others then
            v_id_log := aire.pkg_p_generales.fnc_registrar_log (v_respuesta.nombre_up, aire.pkg_g_seguridad.v_tipo_mensaje_error, 'Error al rechazar la orden: '||sqlerrm);
            sb_escribir_respuesta(1, 'Error N '||v_id_log||' al rechazar orden');
            rollback;
    end prc_rechazar_orden_scr;
-- ult modificacion AM 06/02/2024 10:39 AM
end pkg_g_ordenes;
