namespace Infrastructure.Services
{
    ...
    public class CargaMasivaScrAsigTecnicoService : ICargaMasivaConfiguracion
    {
        #region Parameters
        ...
        public CargaInicial CargaInicial => CargaInicial.ScrAsigTecnico;
        private readonly IWebHostEnvironment _environment;
        private readonly ParametrosCarguesOptions _parametrosCarguesOptions;
        #endregion

        public CargaMasivaScrAsigTecnicoService(
        ...
        public async Task<ResponseDto<MedirTiempo>> Procesar(object parameter, int myUser)
		{
            ...
            {           
                ...
                QueryOp360FileBase64 parameters = new()
                {
                    ...
                    Tipos_Soporte = TipoSoporteEnum.SASGT.ToString(),
                    id_usuario = myUser
                };
                //-- Se validan las columnas del archivo antes de guardarlo en disco.
                int validacion = Funciones.fnValidarExcel(excelFileBytes, _parametrosCarguesOptions.MasivoAsigTecnicoClmns);
                ...
                var listTest2 = await _op360Service.Procesar_Masivo_Asignar_Tecnicos_SCR(tmp);                
                stopwatch.Stop();
                tiempoTotal += stopwatch.ElapsedMilliseconds;
        ...
        private async Task<int> ProcessData(byte[] excelFileBytes, int id_soporte, int myUser, Dictionary<string, Type> columnDefinitions)
        {
            ...
            using (var excelPackage = new ExcelPackage(new MemoryStream(excelFileBytes)))
            {
                ...
                for (int ix = 1; ix <= cnt; ix++)
                {
                    ...
                    await _dynamicBulkService.BulkCopyAsync("ORD_ORDENES_TEMPORAL_ASIGNACION_TECNICO", columnDefinitions, data);
