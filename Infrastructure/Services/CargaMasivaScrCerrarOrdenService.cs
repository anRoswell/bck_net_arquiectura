namespace Infrastructure.Services
{
    ...
    public class CargaMasivaScrCerrarOrdenService : ICargaMasivaConfiguracion
    {
        #region Parameters
        ...
        public CargaInicial CargaInicial => CargaInicial.ScrCerrarOrden;
        private readonly IWebHostEnvironment _environment;
        private readonly ParametrosCarguesOptions _parametrosCarguesOptions;
        #endregion

        public CargaMasivaScrCerrarOrdenService(
        ...
        public async Task<ResponseDto<MedirTiempo>> Procesar(object parameter, int myUser)
		{
            ...
            {           
                ...
                QueryOp360FileBase64 parameters = new()
                {
                    ...
                    Tipos_Soporte = TipoSoporteEnum.SCOM.ToString(),
                    id_usuario = myUser
                };
                //-- Se validan las columnas del archivo antes de guardarlo en disco.
                int validacion = Funciones.fnValidarExcel(excelFileBytes, _parametrosCarguesOptions.MasivoCerrarOrdenClmns);
                ...
                string destinationTable = "ORD_ORDENES_CARGUE_TEMPORAL_CIERRE";
                var columnDefinitions = new Dictionary<string, Type>
                {
                    { "ORDEN", typeof(int) },
                    { "NUMERO_ORDEN", typeof(string) },
                    { "LECTURA", typeof(string) },
                    { "OBSERVACION", typeof(string) },
                    { "CODIGO_TIPO_ORDEN", typeof(string) },
                    { "CODIGO_CAUSAL_CIERRE", typeof(string) },
                    { "ID_SOPORTE", typeof(int) },
                    { "USUARIO_REGISTRA", typeof(string) }
                };
                int registrosExcel = await ProcessData(excelFileBytes, id_soporte, myUser, columnDefinitions, destinationTable);
                stopwatch.Stop();
                tiempoTotal += stopwatch.ElapsedMilliseconds;
                ...
                var listTest2 = await _op360Service.Procesar_Masivo_Cierre_Ordenes_SCR(tmp);                
                stopwatch.Stop();
                tiempoTotal += stopwatch.ElapsedMilliseconds;
        ...
        private async Task<int> ProcessData(byte[] excelFileBytes, int id_soporte, int myUser, Dictionary<string, Type> columnDefinitions, string destinationTable)
        {
            int registrosExcel;
                ...
                for (int ix = 1; ix <= cnt; ix++)
                {
                    ...
                    for (int row = ini; row <= fin; row++)
                    {
                        var rowData = new Dictionary<string, object>
                        {
                            ...
                            { "LECTURA", worksheet.Cells[row, 2].Value?.ToString() },
                            { "OBSERVACION", worksheet.Cells[row, 3].Value?.ToString() },
                            { "CODIGO_TIPO_ORDEN", worksheet.Cells[row, 4].Value?.ToString() },
                            { "CODIGO_CAUSAL_CIERRE", worksheet.Cells[row, 5].Value?.ToString() },
                    ...
                    await _dynamicBulkService.BulkCopyAsync(destinationTable, columnDefinitions, data);
