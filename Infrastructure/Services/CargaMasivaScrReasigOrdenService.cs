namespace Infrastructure.Services
{
    ...
    public class CargaMasivaScrReasigOrdenService : ICargaMasivaConfiguracion
    {
        #region Parameters
        ...
        public CargaInicial CargaInicial => CargaInicial.ScrReasigOrden;
        private readonly IWebHostEnvironment _environment;
        private readonly ParametrosCarguesOptions _parametrosCarguesOptions;
        #endregion

        public CargaMasivaScrReasigOrdenService(
        ...
        public async Task<ResponseDto<MedirTiempo>> Procesar(object parameter, int myUser)
		{
            ...
            {           
                ...
                QueryOp360FileBase64 parameters = new()
                {
                    ...
                    Tipos_Soporte = TipoSoporteEnum.SRAO.ToString(),
                    id_usuario = myUser
                };
                //-- Se validan las columnas del archivo antes de guardarlo en disco.
                int validacion = Funciones.fnValidarExcel(excelFileBytes, _parametrosCarguesOptions.MasivoReasigOrdenClmns);
                ...
                var listTest2 = await _op360Service.Procesar_Masivo_Reasignar_Ordenes_SCR(tmp);                
                stopwatch.Stop();
                tiempoTotal += stopwatch.ElapsedMilliseconds;
                ...
                for (int ix = 1; ix <= cnt; ix++)
                {
                    ...
                    for (int row = ini; row <= fin; row++)
                    {
                        var rowData = new Dictionary<string, object>
                        {
                            ...
                            { "CODIGO_CONTRATISTA", worksheet.Cells[row, 3].Value?.ToString() },
