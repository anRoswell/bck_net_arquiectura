namespace Infrastructure.Services
{
    ...
    public class CargaMasivaScrDesasigTecnicoService : ICargaMasivaConfiguracion
    {
        #region Parameters
        ...
        public CargaInicial CargaInicial => CargaInicial.ScrDesasigTecnico;
        private readonly IWebHostEnvironment _environment;
        private readonly ParametrosCarguesOptions _parametrosCarguesOptions;
        #endregion

        public CargaMasivaScrDesasigTecnicoService(
        ...
        public async Task<ResponseDto<MedirTiempo>> Procesar(object parameter, int myUser)
		{
            ...
            {           
                ...
                int validacion = Funciones.fnValidarExcel(excelFileBytes, _parametrosCarguesOptions.MasivoDesasigTecnicoClmns);
                ...
                QueryOp360FileBase64 parameters = new()
                {
                    ...
                    Tipos_Soporte = TipoSoporteEnum.SDSGT.ToString(),
                    id_usuario = myUser
                };
                ...
                var columnDefinitions = new Dictionary<string, Type>
                {
                    ...
                    {"NIC", typeof(string) },
                    {"TIPO_OS", typeof(string) },
                    //{"USUARIO", typeof(string) },
                    //{"NOMBRE_USUARIO", typeof(string) },
                ...
                stopwatch.Reset();
                stopwatch.Start();
                QueryOp360CargueMasivoValidacionContratistas tmp = new()
                {
                    id_soporte = filesReq.Datos.Id_Soporte,
                    usuario_registra = myUser.ToString(),
                    nombre_archivo = filesReq.Datos.NombreInterno,
                    ind_areacentral = parameter2.ind_areacentral?.ToUpper()
                };
                var listTest2 = await _op360Service.Procesar_Masivo_DesAsignar_Tecnicos_SCR(tmp);                
                stopwatch.Stop();
                tiempoTotal += stopwatch.ElapsedMilliseconds;
                ...
                for (int ix = 1; ix <= cnt; ix++)
                {
                    ...
                    for (int row = ini; row <= fin; row++)
                    {
                        var rowData = new Dictionary<string, object>
                        {
                            ...
                            {"NIC", worksheet.Cells[row, 2].Value?.ToString() },
                            {"TIPO_OS", worksheet.Cells[row, 3].Value?.ToString() },
                            //{"USUARIO", worksheet.Cells[row, 4].Value?.ToString() },
                            //{"NOMBRE_USUARIO", worksheet.Cells[row, 5].Value?.ToString() },
                    ...
                    await _dynamicBulkService.BulkCopyAsync("ORD_ORDENES_TEMPORAL_DESASIGNACION_TECNICO", columnDefinitions, data);
