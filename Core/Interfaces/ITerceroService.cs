...
namespace Core.Interfaces
{
    public interface ITerceroService
    {
        ...
        Task<List<TerceroDto>> GetFiltroTercero(string idLocalidad, int idCategoriaServicio, int idEstado, string razonSocial, string nit, int idPathFileServer);
        Task<TerceroDetalle> GetTerceroDetallePorID(int id, int idPathFileServer);
        ...
        Task<List<ResponseActionUrl>> PostCrear(TerceroCrearDto proveedor, List<PrvDocumento> documentos, string Generatedpassword, string pathFs, string pathFSWeb, SignalParams<ProveedorHub, DataProveedores> signalParam, int idPathFileServer);
        Task<List<ResponseActionUrl>> PutActualizar(TerceroActualizarDto proveedor, List<PrvDocumento> documentos, List<PrvDocumento> listDocumentsOthers, string pathFs, string pathFSWeb, SignalParams<ProveedorHub, DataProveedores> signalParam, int idPathFileServer);
        Task<List<ResponseAction>> PutUpdateEstadoDocument(QueryUpdateEstadoDocument data);
        Task<List<ResponseAction>> PutTerceroAprobar(QueryUpdateEstadoPrv updEstPrv, string pathFs, int idPathFileServer);
